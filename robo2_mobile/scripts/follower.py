    ...
    def publish(self):
        ...
        while not rospy.is_shutdown():
            ...
            if sonar_front > 0.2:
                ...
                if sonar_frontleft < 0.2:
                    self.velocity.linear.x = 0
                    omega = sonar_left 
                    self.velocity.angular.z = 0.1

                elif sonar_frontright < 0.2:
                    self.velocity.linear.x = 0
                    self.velocity.angular.z = 0.1

                elif sonar_left < 0.25:
                    self.velocity.linear.x = 0
                    self.velocity.angular.z = 0.1

                elif sonar_right < 0.25:
                    self.velocity.linear.x = 0
                    self.velocity.angular.z = 0.1
            ...
            sonar_front = rospy.Subscriber('/sensor/sonar_F', Range, self.sonar_front_callback, queue_size=1)
            sonar_frontleft = rospy.Subscriber('/sensor/sonar_FL', Range, self.sonar_frontleft_callback, queue_size=1)
            sonar_frontright = rospy.Subscriber('/sensor/sonar_FR', Range, self.sonar_frontright_callback, queue_size=1)
            sonar_left = rospy.Subscriber('/sensor/sonar_L', Range, self.sonar_left_callback, queue_size=1)
            sonar_right = rospy.Subscriber('/sensor/sonar_R', Range, self.sonar_right_callback, queue_size=1)
